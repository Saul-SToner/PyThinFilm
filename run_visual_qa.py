...
from thinfilm.plotting import apply_plot_style, save_publication_figure
...
def make_contact_sheets(records: list[dict[str, object]], output_dir: Path, *, per_sheet: int = 12) -> list[str]:
    ...
    for page, start in enumerate(range(0, len(records), per_sheet), start=1):
        ...
        save_publication_figure(fig, sheet, dpi=160, max_width_in=None, add_labels=False)
        plt.close(fig)
...
def main() -> None:
    ...
    for root in args.roots:
        ...
        files.extend(path for path in sorted(expanded.rglob("*.png")) if "visual_qa" not in path.parts)
    records = [inspect_png(path) for path in files]
    ...
    fields = ["path", "name", "width_px", "height_px", "aspect", "content_fraction", "margin_left_px", "margin_right_px", "margin_top_px", "margin_bottom_px", "sha256", "issues"]
